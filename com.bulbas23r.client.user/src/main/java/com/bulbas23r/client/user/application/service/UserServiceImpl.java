...
import com.bulbas23r.client.user.infrastructure.persistence.UserQueryRepositoryImpl;
...
public class UserServiceImpl implements UserService {
  ...
  public UserResponseDto getUserDetail(String username) {
    User user = userRepository.findByUsername(username).orElseThrow(() -> new NotFoundException("존재하지 않는 유저입니다."));
    //dto 변환
    UserDataDto userDataDto = new UserDataDto(user);
  ...
  public void updateUser(String username, UserPatchRequestForRegisterDto userRequestDto) {
    userRepository.findByUsername(username).orElseThrow(() -> new NotFoundException("존재하지 않는 유저입니다."));
